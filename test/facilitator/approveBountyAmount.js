...
describe('Facilitator.approveBountyAmount()', () => {
    ...
    const setup = function() {
        ...
        mockBaseTokenContract = sinon.mock(facilitator.contracts.BaseToken(baseTokenAddress));
        const baseTokenContract = mockBaseTokenContract.object;

        // Mock approve transaction object.
        mockTx = sinon.mock(baseTokenContract.methods.approve(gatewayAddress, bountyAmount));
        ...
        spyBaseTokenApprove = sinon.replace(baseTokenContract.methods, 'approve', sinon.fake.returns(mockTx.object));

        // Fake the Base token contract;
        sinon.replace(facilitator.contracts, 'BaseToken', sinon.fake.returns(baseTokenContract));
    ...
    it('should approve bounty amount with default gas value', async function() {
        ...
        assert.strictEqual(result.txOptions.from, facilitatorAddress, 'From address must be facilitator address.');
        assert.strictEqual(result.txOptions.to, baseTokenAddress, 'To address must be base token address.');
    ...
    it('should approve bounty amount when gas amount is provided in argument', async function() {
        ...
        // Call approve.
        const result = await facilitator.approveBountyAmount(facilitatorAddress, gas);
