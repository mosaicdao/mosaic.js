...
const _ = require('underscore'),
  ...
  // core = require('web3-core'),
  utils = require('web3-utils');
// Subscription = require('web3-core-subscriptions').subscription,
// formatters = require('web3-core-helpers').formatters,
// errors = require('web3-core-helpers').errors
...
module.exports = function(MosaicWeb3Prototype) {
  ...
  MosaicWeb3Prototype._signTx = function(promiEvent, txToBeSigned, callback) {
    const oWeb3 = this;
  ...
  MosaicWeb3Prototype._bindPromieEvents = function(sourcePromieEvent, destinationPromieEvent) {
    let isSourcePomiseRejected = false;
    sourcePromieEvent
      .catch(function(reason) {
        //Reject out destinationPromieEvent.
        isSourcePomiseRejected = true;
        destinationPromieEvent.reject.apply(destinationPromieEvent, arguments);
      })
      .then(function() {
        ...
        destinationPromieEvent.resolve.apply(destinationPromieEvent, arguments);
      });

    //Override emit method.
    let org_emit_scope = sourcePromieEvent;
    ...
    org_emit_scope.emit = function() {
      ...
      let destinationReturnVal = destinationPromieEvent.eventEmitter.emit.apply(
        destinationPromieEvent.eventEmitter,
        arguments
      );
