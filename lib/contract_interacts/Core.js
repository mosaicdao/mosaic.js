...
const Core = function(originCoreAddress, originOptions, auxiliaryCoreAddress, auxilaryOptions) {
  ...
  if (auxiliaryCoreAddress) {
    let auxiliaryCoreContract = new auxiliaryWeb3Obj.eth.Contract(
      coreJsonInterface,
      auxiliaryCoreAddress,
      auxilaryOptions || {}
    );
...
const proto = {
  ...
  init: function(originCoreContract, auxiliaryCoreContract) {
    const oThis = this;

    oThis._getOriginContract = function() {
      return originCoreContract;
    };
    oThis._getAuxiliaryContract = function() {
      return auxiliaryCoreContract;
    };
