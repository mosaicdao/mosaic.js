...
InitCore.prototype = {
  ...
  perform: async function() {
    ...
    let auxiliaryCoreContractAddress = await oThis.deployCoreOnAuxiliary();

    await oThis.deployWorkerOnOrigin();

    await oThis.deployCoreOnOrigin(auxiliaryCoreContractAddress);
  },
  ...
  deployWorkerOnOrigin: async function () {
    ...
    let valueWorkerContractDeployResponse = await deployContract(valueWeb3, 'Workers', oThis.originConfig.deployerAddress,
      oThis.gasPrice, ['0xf18b97b312EF48C5d2b5C21c739d499B7c65Cf96']);
    ...
    let setOpsAddressForValueWorkersResponse = await valueWorkerContract.methods.setOpsAddress(oThis.originConfig.opsAddress).send({
      from: oThis.originConfig.deployerAddress, gasPrice: oThis.gasPrice });

    console.log('setOpsAddress on value chain Workers receipt:', JSON.stringify(setOpsAddressForValueWorkersResponse, null, 4));
    ...
    let setWorkerForValueWorkersResponse = await valueWorkerContract.methods.setWorker(
      oThis.originConfig.workerAddress, '10000000000000000000000000000000000').send({
      from: oThis.originConfig.opsAddress, gasPrice: oThis.gasPrice });

    console.log('setWorker on value chain Workers receipt:', JSON.stringify(setWorkerForValueWorkersResponse, null, 4));
  },

  deployCoreOnOrigin: async function (auxiliaryCoreContractAddress) {
    const oThis = this;
    ...
    let blockHeigth = latestBlock.number
    ...
    let coreDeployParams = [
      oThis.originConfig.registrar,
      oThis.originConfig.chainId,
      oThis.originConfig.chainIdRemote,
      oThis.originConfig.openSTRemote,
      oThis.originConfig.remoteChainBlockGenerationTime,
      blockHeigth,
      stateRoot,
      oThis.valueWorkerContractAddress
    ];

    let valueCoreDeployResponse = await deployContract(valueWeb3, 'Core', oThis.originConfig.deployerAddress,
      oThis.gasPrice, coreDeployParams);

    console.log('valueCoreDeployResponse:', valueCoreDeployResponse);

    let valueCoreContract = valueCoreDeployResponse.instance
      , valueCoreContractAddress = valueCoreDeployResponse.receipt.contractAddress
    ;
    console.log('setting CoCore address', auxiliaryCoreContractAddress);
    let setCoCoreOnOriginResponse = await valueCoreContract
      .methods
      .setCoCoreAddress(auxiliaryCoreContractAddress)
      .send({
        from: oThis.originConfig.deployerAddress,
        gasPrice: oThis.gasPrice
      });
    console.log("setCoCoreOnOriginResponse  ", setCoCoreOnOriginResponse);
  },
  ...
  deployWorkerOnAuxiliary: async function () {
    ...
    let auxiliaryWorkerContractDeployResponse = await deployContract(auxiliaryWeb3, 'Workers', oThis.auxliaryConfig.deployerAddress,
      oThis.gasPrice, ['0xf18b97b312EF48C5d2b5C21c739d499B7c65Cf96']);
    ...
    let setOpsAddressForAuxiliaryWorkersResponse = await auxiliaryWorkerContract.methods.setOpsAddress(oThis.auxliaryConfig.opsAddress).send({
      from: oThis.auxliaryConfig.deployerAddress, gasPrice: oThis.gasPrice });

    console.log('setOpsAddress on auxiliary chain Workers receipt:', JSON.stringify(setOpsAddressForAuxiliaryWorkersResponse, null, 4));
    ...
    let setWorkerForAuxiliaryWorkersResponse = await auxiliaryWorkerContract.methods.setWorker(
      oThis.auxliaryConfig.workerAddress, '10000000000000000000000000000000000').send({
      from: oThis.auxliaryConfig.opsAddress, gasPrice: oThis.gasPrice });

    console.log('setWorker on auxiliary chain Workers receipt:', JSON.stringify(setWorkerForAuxiliaryWorkersResponse, null, 4));
  },
  ...
  deployCoreOnAuxiliary: async function () {
    ...
    let coreDeployParams = [
      oThis.auxliaryConfig.registrar,
      oThis.auxliaryConfig.chainId,
      oThis.auxliaryConfig.chainIdRemote,
      oThis.auxliaryConfig.openSTRemote,
      oThis.auxliaryConfig.remoteChainBlockGenerationTime,
      blockHeigth,
      stateRoot,
      oThis.auxiliaryWorkerContractAddress
    ];

    let auxiliaryCoreDeployResponse = await deployContract(auxiliaryWeb3, 'Core', oThis.auxliaryConfig.deployerAddress,
      oThis.gasPrice, coreDeployParams);
    ...
    return auxiliaryCoreContractAddress;
  }
};
