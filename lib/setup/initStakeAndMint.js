...
const path = '/Users/sarveshjain/workspace/sj/mosaic.js/mosaic-setup/originChainData/';
...
StakeAndMint.prototype = {
  perform: async function() {
    ...
    const stakeResponse = await oThis._stake(stakeAmount, beneficiary, staker, gasPrice, nonce, hashLock, signature);
  ...
  _signHash: async function(hash, nonce, organization) {
    const oThis = this;
    let typeHash = await oThis.gateway.methods.stakeRequestTypeHash().call({
  ...
  _getNonce: async function(address) {
    const nonce = await oThis.erc20Gateway.origin.nonce(address).call({
  ...
  _stake: async function(amount, beneficiary, staker, gasPrice, nonce, hashLock, signature) {
    let stakeResponse = await oThis.erc20Gateway
      .stake(stakeAmount, beneficiary, staker, gasPrice, nonce, hashLock, signature)
      .call({
        from: oThis.config.originDeployerAddress,
        gas: 4700000,
        gasPrice: 1000
