...
InitMessageBus.prototype = {
  perform: async function() {
    ...
    await oThis.deployMessageBusOnOrigin();

    await oThis.deployMessageBusOnAuxiliary();
  ...
  deployMessageBusOnOrigin: async function () {
    ...
    let valueMessageBusLibraryDeployResponse = await deployContract(valueWeb3, 'MessageBus', oThis.originConfig.deployerAddress,
      oThis.gasPrice, []);

    console.log('valueMessageBusLibraryDeployResponse:', valueMessageBusLibraryDeployResponse);
  },
  ...
  deployMessageBusOnAuxiliary: async function () {
    ...
    let auxiliaryMessageBusLibraryDeployResponse = await deployContract(auxiliaryWeb3, 'MessageBus', oThis.auxliaryConfig.deployerAddress,
      oThis.gasPrice, []);

    console.log('auxiliaryMessageBusLibraryDeployResponse:', auxiliaryMessageBusLibraryDeployResponse);
  }
