...
Setup.prototype = {
   ...
   *
   * @param originConfig {object} - config related to origin chain
   * @param originMessageBusAddress - origin chain MessageBus library address
   * @param auxliaryConfig - config related to auxiliary chain
   * @param auxiliaryMessageBusAddress - auxiliary chain MessageBus library address
   * @returns {promise}
   */
  initGateway: function(originConfig, originMessageBusAddress, auxliaryConfig, auxiliaryMessageBusAddress) {
    const oThis = this,
      InitGateway = oThis.ic().InitGateway();

    return new InitGateway(originConfig, originMessageBusAddress, auxliaryConfig, auxiliaryMessageBusAddress).perform();
  }
};
