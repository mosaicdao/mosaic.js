...
Setup.prototype = {
   ...
   * @param originConfig
   * @param auxliaryConfig
   * @return {*|Promise<*>|TransactionObject<Contract>|Promise<void>}
   */
  deployGateway: function(originConfig, auxliaryConfig) {
    const oThis = this,
  ...
  linkGateways: function(config) {
    const oThis = this,
      LinkGateway = oThis.ic().LinkGateways();

    return new LinkGateway(config).link();
  }
};
